...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Check for pending bookings and display notification dot if needed
  ...
  window.addEventListener("scroll", function () {
    if (window.scrollY > 50) {
      ...
      document
        .querySelectorAll(".menu-button button, .login-button button")
        .forEach((button) => {
  ...
  if (searchButton) {
    searchButton.addEventListener("click", function () {
      ...
      const destination = document.querySelector(
        ".search-item:nth-child(1) p"
      ).textContent;
      const dates = document.querySelector(
        ".search-item:nth-child(2) p"
      ).textContent;
      const guests = document.querySelector(
        ".search-item:nth-child(3) p"
      ).textContent;
  ...
  buttons.forEach((button) => {
    button.addEventListener("mouseenter", function () {
      ...
      if (
        this.classList.contains("search-button") ||
        this.classList.contains("promo-button")
      ) {
  ...
  window.addEventListener("scroll", function () {
    ...
    if (heroBg) {
      heroBg.style.transform = `translateX(-50%) translateY(${
        scrollPosition * 0.2
      }px)`;
    }
  });
  ...
  if (
    ...
    typeof jQuery().carousel === "function"
  ) {
    $(".carousel").carousel({
      interval: 5000,
    });
...
/**
 * Checks if the current user has any pending bookings and displays the notification dot if true
 */
function checkPendingBookings() {
  // Only run if user is logged in
  const userContainer = document.querySelector(".user-container");
  if (!userContainer) return; // User is not logged in

  // Get the notification dot element
  const notificationDot = document.querySelector(".notification-dot");
  if (!notificationDot) return; // Dot element not found

  // Make AJAX request to check for pending bookings
  fetch("../api/check_pending_bookings.php")
    ...
    .then((data) => {
      if (data.hasPendingBookings) {
        // Display the notification dot if there are pending bookings
